...
func TestAssistant_Run(t *testing.T) {
	...
	message, err := assistant.Run[string, string](ctx, &asst, &thread,
		"What's the weather in San Francisco today and the likelihood it'll rain?",
	)
	assert.NoError(t, err)
	assert.True(t, thread.ID != "")
	assert.Equal(t, "The current temperature in San Francisco, CA is 72°F. The likelihood of rain today is 20%.", message)

	message, err = assistant.Run[string, string](ctx, &asst, &thread,
		"What's the weather in New York City?",
	)
	assert.NoError(t, err)
	assert.Equal(t, "The current temperature in New York City, NY is 72°F. The likelihood of rain today is 20%.", message)
}
